...
AnaInput::AnaInput( string datacardInput ) {
  ...
  std::cout << "created object of class AnaInput with datacard: " << datacardInput << std::endl;
...
AnaInput::~AnaInput(){
  ...
  std::cout << "deleting object of class AnaInput with datacard: " << datacardfile  << std::endl;
...
void AnaInput::GetParameters(string paraName, int* thePara, string cfgFile ){
     ...
     if ( !paraFile.is_open() )  cout<<"[AnaInput::GetParameters] error while opening data card (" << cfgFile << ") => check file path and the folder "<<endl;
     string  line;
     string  getName;
     string  getValue;
     size_t  pos ;
