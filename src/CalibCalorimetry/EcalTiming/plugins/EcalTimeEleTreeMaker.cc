...
//  Maintained by : Tambe E. Norbert (UMN)
//
...
EcalTimeEleTreeMaker::EcalTimeEleTreeMaker (const edm::ParameterSet& iConfig) :
  ...
  gsfElectrons_                            (iConfig.getParameter<edm::InputTag> ("gsfElectrons")),
...
//  trackCollectionTag_                      (iConfig.getParameter<edm::InputTag> ("trackSource")),
  naiveId_ (0)              
{
...
*/
 ...
 // get collection of GsfElectrons
/*  edm::Handle<reco::GsfElectronCollection> gsfElectrons;
  iEvent.getByLabel(gsfElectrons_, gsfElectrons);
  if ( ! gsfElectrons.isValid()) {      
    std::cout << "[EcalTimeEleTreeMaker] No electrons found in this event with tag "  << gsfElectrons << std::endl;     return ; }
  else {      if( 0 ) std::cout  << "[EcalTimeEleTreeMaker] got electrons collection with tag: " << gsfElectrons_ << "\t"<< gsfElectrons << std::endl; }
  const reco::GsfElectronCollection& gsfElecs = *(gsfElectrons.product());
*/
...
void EcalTimeEleTreeMaker::dumpBarrelClusterInfo (const edm::Event& iEvent,
						  ...
						  //const reco::GsfElectronCollection& gsfElecs,
...
{            
  ...
  //unsigned nrecopho = photons->size();
  unsigned nrecoSC = theBarrelSuperClusters->size();
...
*/ 
  ...
  //loop on all Barrelsuperclusters in event
  for (reco::SuperClusterCollection::const_iterator sclus = theBarrelSuperClusters->begin () ; 
       ...
       ++sclus) 
    {
        ...
        // bool foundEBMatchingElectron(false);
	//use SuperCluster-UncleanedOnlyGsfElectron Matching Technique
	double  et   = (sclus->position().eta() == 0 )? 0 : sclus->energy()/TMath::CosH(sclus->position().eta() ) ;
	//double  eta  = sclus->position().eta() ;
	//double  phi  = sclus->position().phi() ;
        //double  enr  = sclus->energy() ; 
	...
	//SCluster4Vector  = SclusP4EB ; 
       ...
       // for (eleIt=gsfElecs.begin(); eleIt!=gsfElecs.end() && numberOfSuperClusters<MAXSC; eleIt++) {
       // for (eleIt=gsfElecs->begin(); eleIt!=gsfElecs->end() && numberOfSuperClusters<MAXSC; eleIt++) {	 
        

         //double maxPt = 0 ;
         //for(reco::PhotonCollection::const_iterator phoIt = photons->begin(); phoIt != photons->end(); phoIt++) {

            //if ( !( phoIt->hasPixelSeed () ) ) continue ;
            //if ( ( phoIt->hasPixelSeed () ) ) continue ;
	    
	    //if (phoIt->pt() < elePtCut_) continue ;
	    if (et < elePtCut_) continue ;
            // remove electrons which are too soft
            //if (eleIt->pt() < elePtCut_) continue;
     	    //Appply very loose cut based selection
    	/* 
            if(!eleIt->passingCutBasedPreselection()) continue;
            //check for  Standard Cutbased Identification ie VBTF90 Loose
             if( eleIt->scSigmaIEtaIEta() < 0.5 ) continue;
             if( eleIt->deltaEtaSuperClusterTrackAtVtx() < 0.5 ) continue;
             if( eleIt->deltaPhiSuperClusterTrackAtVtx() < 0.7 ) continue;
             if( eleIt->hadronicOverEm() < 0.7 ) continue;
         */
	  /*  
	    //if( ROOT::Math::VectorUtil::DeltaR( eleIt->p4(), SCluster4Vector )  < 0.05 ) { foundEBMatchingElectron = true;  nEleEB++ ; }
	     if( ROOT::Math::VectorUtil::DeltaR( eleIt->p4(), SCluster4Vector )  > 0.5 ) {  nEleEB++ ; continue; }
  	     //if ( delR  >  eledeltaRCut_ ) { nEle++ ; continue ; }
          */      
             //Get supercluster reference of Physics Objects    
             //reco::SuperClusterRef sclusRef=phoIt->superCluster();
             //reco::SuperClusterRef sclusRef=eleIt->pflowSuperCluster();
             //reco::SuperClusterRef sclusRef=eleIt->superCluster();
             //reco::SuperClusterRef sclusRef=eleIt->core()->superCluster();
             //if (sclusRef.isNull()) { std::cout  << "[EcalTimeEleTreeMaker] reference to superlcuster is NULL: we have a problem!" << std::endl; continue; }
             if (0) { std::cout  << "[EcalTimeEleTreeMaker] No SuperCluster found in Event: we have a problem!" << std::endl; continue; }
          
	     //reco::SuperClusterRef sclus=eleIt->pflowSuperCluster(); 
             //reco::SuperClusterRef sclus=eleIt->superCluster();
             //reco::SuperClusterRef sclus=eleIt->core()->superCluster();
   
            // reco::SuperClusterRef sclus=phoIt->superCluster();
             //std::cout  << "uncleanedPhoton's [eta, phi] : = " << "["  << phoIt->eta() << " ," << phoIt->phi()  << "]"  << std::endl;
             std::cout  << "uncleanedSuperCluster's [eta, phi] : = " << "["  << sclus->position().eta() << " ," << sclus->position().phi()  << "]"  << std::endl;
             //std::cout  << "uncleanedgsfElectron's [eta, phi] : = " << "["  << eleIt->eta() << " ," << eleIt->phi()  << "]"  << std::endl;
	 
	     double eta_det=sclus->position().eta();
	    ...
	    /* if( 0 ) std::cout  << "[EcalTimeEleTreeMaker] BEF got ONE pat electron with SC-pt: " 
		       ...
		       << std::endl; 
            */
	       if(!(nrecoSC = 0)) std::cout  << "[EcalTimeEleTreeMaker] YEAH! Got one Uncleaned reco electron Candidate with Pt: " 
    		       << et << " eta: " << eta_det << " and with ID : " //<<  eleIt->electronID( workingPoint_.c_str()  ) 
		       << std::endl; 
       ...
       */
        ...
        //if(!eleIt->isEB()) { continue;
        //if(!phoIt->isEB()) { continue;
        if(!(eta_det < 1.479)) { continue;
        } else {
                  
                 myTreeVariables_.nBarrelSuperClusters +=1;
		 nSCEB += 1; 
               //If using superclusters
		 ...
		 myTreeVariables_.superClusterRawEnergy[numberOfSuperClusters] = sclus->rawEnergy () ;
               ...
               */
               ...
               /*
		 myTreeVariables_.nClustersInSuperCluster[numberOfSuperClusters] = sclus.get()-> clustersSize () ;
      		 myTreeVariables_.superClusterEta[numberOfSuperClusters] = eleIt->eta () ;   // Also works!
      		 myTreeVariables_.superClusterPhi[numberOfSuperClusters] = eleIt->phi () ;
     		 myTreeVariables_.superClusterX[numberOfSuperClusters] = sclus.get()->position().x () ;
      		 myTreeVariables_.superClusterY[numberOfSuperClusters] = sclus.get()->position().y () ;
     		 myTreeVariables_.superClusterZ[numberOfSuperClusters] = sclus.get()->position().z () ;
  ...
  } // end of Loop over SuperCluusters
  //number of clusters / xtals for the whole event
  myTreeVariables_.nClusters = numberOfClusters ;
  std::cout <<" Number of SuperClusters Found in EB : " << nSCEB  << std::endl;
  return ;
						  ...
						  EcalTimeTreeContent & myTreeVariables_)
{
  ...
  for (reco::SuperClusterCollection::const_iterator sclus = theEndcapSuperClusters->begin () ; 
	...
	acceptScInHighEta=true;
      ...
      reco::GsfElectronCollection::const_iterator eleIt;
     /// reco::GsfElectronCollection::const_iterator It;
      
      //for (It=patElecs.begin(); It!=patElecs.end(); It++) 
     /* for (It=gsfElecs.begin(); It!=gsfElecs.end(); It++) 
	{
      */
      for(reco::PhotonCollection::const_iterator phoIt = photons->begin(); phoIt != photons->end(); phoIt++) 
	 {
	  ...
	  // remove electrons that don't pass the desired selection
/*	  ector<int>::iterator itEleId = find ( eleIdCuts_.begin(), eleIdCuts_.end(), It->electronID( workingPoint_.c_str()) ) ;
	  if( itEleId == eleIdCuts_.end()) continue;
*/ 
          //Appply very loose based selection
	
//	if(!It->passingCutBasedPreselection()) continue;

           //check for  Standard Cutbased Identification ie VBTF90 Loose
	/* if( It->scSigmaIEtaIEta() < 0.3 ) continue;
	   if( It->deltaEtaSuperClusterTrackAtVtx() < 0.7 ) continue;
	   if( It->deltaPhiSuperClusterTrackAtVtx() < 0.3 ) continue;
	   if( It->hadronicOverEm() < 0.7 ) continue;
         */ 
	  ...
	  //if ( 	      sqrt( ( sclus->position() - It->superClusterPosition () ).Mag2() ) < 1  	      ) 
	  if ( 	      sqrt( ( sclus->position() - phoIt->caloPosition () ).Mag2() ) < 1  	      ) 
	    {
	     ...
	     */
	      myTreeVariables_.superClusterVertexX[numberOfSuperClusters] = phoIt->caloPosition().x() ;
	      myTreeVariables_.superClusterVertexY[numberOfSuperClusters] = phoIt->caloPosition().y() ;
	      myTreeVariables_.superClusterVertexZ[numberOfSuperClusters] = phoIt->caloPosition().z() ;
	      break;
	    }
